...
class AccountAdapter < SourceAdapter
  ...
  def search(params)
    ...
    "<CustomerSelectionByNameAndAddress>
                <Common>
                   <Name>
                      <FirstLineName>#{params['firstLineName']}</FirstLineName>
    ...
    @result = {}
    customerArray = mash.customer_erp_address_basic_data_by_name_and_address_response_sync.customer
    if customerArray.is_a?(Hashie::Mash)
      customerArray = [customerArray]
    end
    customerArray.each { |customer|
      p "Customer: " + customer.to_s
      p "Common: " + customer.common.to_s
      if customer.common? and customer.common.name?
        nameHash = customer.common.name
        if nameHash.first_line_name? and nameHash.second_line_name?
          entry = {}
